...
async function main() {
  ...
  await setRange(sheets, 'itinerary!A1:U1', [ITINERARY_HEADERS]);
  ...
  await clearRange(sheets, 'itinerary!A2:U1000');
  const itinRows = SEED_DAYS.map((d) => [
    ...
    '', // day_summary — fill via inferDayContent.js after the trip
    '', // blog — fill via inferBlogContent.js after the trip
  ]);
  await setRange(sheets, `itinerary!A2:U${1 + itinRows.length}`, itinRows);
